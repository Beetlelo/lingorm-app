// 中国农历转换（1900–2049 查表法）+ ISO 周数
// 仅用于界面展示农历日期与周数，不影响业务逻辑。

const lunarInfo = [
  0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2,
  0x09b68, 0x09b40, 0x0a8a6, 0x0ab60, 0x0a550, 0x0b4a0, 0x0baa4, 0x0b550, 0x0b255, 0x0d6a0,
  0x0ada2, 0x095b0, 0x14977, 0x04970, 0x0a4b0, 0x0b4b5, 0x06a50, 0x06d40, 0x1ab54, 0x02b60,
  0x09570, 0x052f2, 0x04970, 0x06566, 0x0d4a0, 0x0ea50, 0x06e95, 0x05ad0, 0x02b60, 0x186e3,
  0x092e0, 0x1c8d7, 0x0c950, 0x0d4a0, 0x1d8a6, 0x0b550, 0x056a0, 0x1a5b4, 0x025d0, 0x092d0,
  0x0d2b2, 0x0a950, 0x0b557, 0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5b0, 0x14573, 0x052b0,
  0x0a9a8, 0x0e950, 0x06aa0, 0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260, 0x0f263,
  0x0d950, 0x05b57, 0x056a0, 0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558,
  0x0b540, 0x0b5a0, 0x195a6, 0x095b0, 0x049b0, 0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40,
  0x0af46, 0x0ab60, 0x09570, 0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58, 0x055c0,
  0x0ab60, 0x096d5, 0x092e0, 0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552, 0x056a0, 0x0abb7,
  0x025d0, 0x092d0, 0x0cab5, 0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0,
  0x15176, 0x052b0, 0x0a930, 0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0,
  0x0d260, 0x0ea65, 0x0d530, 0x05aa0, 0x076a0, 0x096d5, 0x04af0, 0x04ad0, 0x0a4d0, 0x1d0b6,
  0x0d250, 0x0d520, 0x0dd45, 0x0b5a0, 0x056d0, 0x055b2, 0x049b0, 0x0a577, 0x0a4b0, 0x0aa50,
  0x1b255, 0x06d20, 0x0ada0, 0x14b63, 0x09370, 0x049f8, 0x04970, 0x064b0, 0x168a6, 0x0ea50,
  0x06b20, 0x1a6c4, 0x0aae0, 0x0a2e0, 0x0d2e3, 0x0c960, 0x0d557, 0x0d4a0, 0x0da50, 0x1d5b4,
  0x056a0, 0x0a6d0, 0x055c2, 0x0ab60, 0x096d5, 0x04ae0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558,
  0x0b540, 0x0b5a0, 0x195a6, 0x095b0, 0x049b0, 0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40,
  0x0af46, 0x0ab60, 0x09570, 0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58, 0x055c0,
  0x0ab60, 0x096d5, 0x092e0, 0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552, 0x056a0, 0x0abb7,
  0x025d0, 0x092d0, 0x0cab5, 0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0,
  0x15176, 0x052b0, 0x0a930, 0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0,
  0x0d260, 0x0ea65, 0x0d530, 0x05aa0, 0x076a0, 0x096d5, 0x04af0, 0x04ad0, 0x0a4d0, 0x1d0b6,
  0x0d250, 0x0d520, 0x0dd45, 0x0b5a0, 0x056d0, 0x055b2, 0x049b0, 0x0a577, 0x0a4b0, 0x0aa50,
  0x1b255, 0x06d20, 0x0ada0, 0x14b63, 0x09370, 0x049f8, 0x04970, 0x064b0, 0x168a6, 0x0ea50,
  0x06b20, 0x1a6c4, 0x0aae0, 0x0a2e0, 0x0d2e3, 0x0c960, 0x0d557, 0x0d4a0, 0x0da50, 0x1d5b4,
  0x056a0, 0x0a6d0, 0x055c2, 0x0ab60, 0x096d5, 0x04ae0,
]

const lunarMonths = ['正', '二', '三', '四', '五', '六', '七', '八', '九', '十', '冬', '腊']
const lunarDays = [
  '初一', '初二', '初三', '初四', '初五', '初六', '初七', '初八', '初九', '初十',
  '十一', '十二', '十三', '十四', '十五', '十六', '十七', '十八', '十九', '二十',
  '廿一', '廿二', '廿三', '廿四', '廿五', '廿六', '廿七', '廿八', '廿九', '三十',
]

function lYearDays(y: number): number {
  let sum = 348
  for (let i = 0x8000; i > 0x8; i >>= 1) sum += lunarInfo[y - 1900] & i ? 1 : 0
  return sum + leapDays(y)
}

function leapDays(y: number): number {
  if (leapMonth(y)) {
    return lunarInfo[y - 1900] & 0x10000 ? 30 : 29
  }
  return 0
}

function leapMonth(y: number): number {
  return lunarInfo[y - 1900] & 0xf
}

function monthDays(y: number, m: number): number {
  return lunarInfo[y - 1900] & (0x10000 >> m) ? 30 : 29
}

export type LunarDate = {
  month: number
  day: number
  isLeap: boolean
  monthText: string
  dayText: string
}

export function solar2lunar(year: number, month: number, day: number): LunarDate | null {
  if (year < 1900 || year > 2049) return null
  let offset = Date.UTC(year, month - 1, day) - Date.UTC(1900, 0, 31)
  offset = Math.round(offset / 86400000)
  let temp = 0
  let y = 1900
  for (y = 1900; y < 2050 && offset > 0; y++) {
    temp = lYearDays(y)
    offset -= temp
  }
  if (offset < 0) {
    offset += temp
    y--
  }
  const leap = leapMonth(y)
  let isLeap = false
  let m = 1
  for (m = 1; m < 13 && offset > 0; m++) {
    if (leap > 0 && m === leap + 1 && !isLeap) {
      m--
      isLeap = true
      temp = leapDays(y)
    } else {
      temp = monthDays(y, m)
    }
    if (isLeap && m === leap + 1) isLeap = false
    offset -= temp
  }
  if (offset === 0 && leap > 0 && m === leap + 1) {
    if (isLeap) {
      isLeap = false
    } else {
      isLeap = true
      m--
    }
  }
  if (offset < 0) {
    offset += temp
    m--
  }
  const lunarMonth = m
  const d = offset + 1
  return {
    month: lunarMonth,
    day: d,
    isLeap,
    monthText: (isLeap ? '闰' : '') + lunarMonths[lunarMonth - 1] + '月',
    dayText: d === 1 ? (isLeap ? '闰' : '') + lunarMonths[lunarMonth - 1] + '月' : lunarDays[d - 1],
  }
}

// 短标签：农历初一显示月份名，其余显示日
export function lunarShort(year: number, month: number, day: number): string {
  const l = solar2lunar(year, month, day)
  if (!l) return ''
  return l.dayText
}

// ISO 周数（周一为一周起点）
export function isoWeekNumber(date: Date): number {
  const d = new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate()))
  const dayNum = (d.getUTCDay() + 6) % 7
  d.setUTCDate(d.getUTCDate() - dayNum + 3)
  const firstThursday = d.getTime()
  d.setUTCMonth(0, 1)
  if (d.getUTCDay() !== 4) {
    d.setUTCMonth(0, 1 + ((4 - d.getUTCDay() + 7) % 7))
  }
  return 1 + Math.ceil((firstThursday - d.getTime()) / 604800000)
}
